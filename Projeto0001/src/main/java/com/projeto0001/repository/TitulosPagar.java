...
/*
	...
	public List<Pedido> filtrados(PedidoFilter filtro) {
		...
		Criteria criteria = session.createCriteria(Pedido.class)
				...
				//fazemos uma assosciação (join) com vendedor e nomeamos com "v"
		...
		if(filtro.getNumeroAte() != null){
			// id deve ser menor ou igual (le = greater or equals) a filtro.numeroAte
		...
		if(filtro.getStatuses() != null && filtro.getStatuses().length > 0){
			// adicionamos uma restricao "in", passando um array de constantes da enum StatusPedido
	...
	*/
	// salvar o Titulo a pagar no banco de dados. Inlusão e atualização
	public TituloPagar guardar(TituloPagar tituloPagar){
		return this.manager.merge(tituloPagar);
	}

	// buscar titulo por ID
	public TituloPagar porId(Long id) {
		return this.manager.find(TituloPagar.class, id);
	}
}
